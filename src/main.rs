...
fn remove_todo(idt: Uuid) {
    ...
    for (i, todo) in lock.iter().enumerate() {
        if todo.id == idt {
            idx = i;
            let num_deleted = diesel::delete(tasks.filter(task.eq(&todo.name[..])))
                .execute(&connection)
                .expect("Error deleting todo item");
            println!("Deleted {} todo", num_deleted);
...
fn toggle_todo(id: Uuid) {
    ...
    for todo in &mut *lock {
        if todo.id == id {
            todo.done = !todo.done;
            toggle_todo_done_db(todo);
        }
    }
}


fn toggle_todo_done_db(todo : &mut Todo) {
    let connection = establish_connection();
    use todo::schema::tasks::dsl::*;
    let res = diesel::update(tasks.filter(task.eq(&todo.name[..]))) 
            .set(done.eq(todo.done))
            .execute(&connection)
            .expect(&format!("Unable to find todo {}", todo.id));
    println!("Completion status of Todo item {} is {}",todo.name,todo.done);        
}
// Routes
